...
def execute(room, messageHandle):
    if room.gameStatus == GameStatus.WAITING:
        ...
        if (1 != room.banker and room.xiazhuang) or (
                banker is not None and banker.shangzhuangScore < int(config.get("hongbao", "getBankerScore"))):
            messageHandle.broadcast_watch_to_gateway(LEAVE_BANKER, None, room)
        if bool(config.get("hongbao", "onlyPlayerBanker")) and 0 == len(room.bankerList) and (
        ...
        if bool(config.get("hongbao", "onlyPlayerBanker")) and 1 == room.banker:
            gl.get_v("serverlogger").logger.info('''开始游戏-无庄''')
            return
