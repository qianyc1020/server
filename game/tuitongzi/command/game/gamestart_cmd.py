...
def execute(room, messageHandle):
    if room.gameStatus == GameStatus.WAITING:
        ...
        getBankerScore = int(config.get("tuitongzi", "getBankerScore"))
        onlyPlayerBanker = bool(config.get("tuitongzi", "onlyPlayerBanker"))
        ...
        if onlyPlayerBanker and 0 == len(room.bankerList) and (
                1 == room.banker or room.xiazhuang or banker is None or room.bankerScore < getBankerScore):
